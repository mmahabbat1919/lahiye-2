...
class DOMManipulator {
  ...
  _init() {
    ...
    this._sortBtn.addEventListener("click", () => this._handleSort());
    this._sortDir = true;
  ...
  displayTodos() {
    ...
    const items = todos.map((t) => {
      ...
      item.className = "li-lists";
      const todoInput = document.createElement("input");
      todoInput.className = "todo-list-input";
  ...
  _handleAdd() {
    try {
      ...
      this._sortDir = true; //bunu yazmayada bilersen
      this.displayTodos();
    } catch (error) {
      this._showError(error.message);
    }
